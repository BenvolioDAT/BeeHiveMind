...
function loadCombatMeleeTiersFromSpec() {
  if (!TaskSpawn || typeof TaskSpawn.getTierList !== 'function') {
    return null;
  }
  var list;
  try {
    list = TaskSpawn.getTierList('CombatMelee');
  } catch (err) {
    list = null;
  }
...
var FALLBACK_COMBAT_MELEE_BODY_TIERS = [
  [
...
var COMBAT_MELEE_BODY_TIERS = loadCombatMeleeTiersFromSpec() || FALLBACK_COMBAT_MELEE_BODY_TIERS.map(function (tier) {
  return tier.slice();
});
...
module.exports.getTierList = function () {
  if (TaskSpawn && typeof TaskSpawn.getTierList === 'function') {
    try {
      var central = TaskSpawn.getTierList('CombatMelee');
  ...
  var fallback = [];
  for (var i = 0; i < COMBAT_MELEE_BODY_TIERS.length; i++) {
    var parts = COMBAT_MELEE_BODY_TIERS[i];
...
module.exports.getSpawnBody = function (energyOrRoom, roomOrContext, maybeContext) {
  ...
  if (TaskSpawn && typeof TaskSpawn.getBodyFor === 'function') {
    try {
      var info = TaskSpawn.getBodyFor('CombatMelee', room, context) || {};
      if (info && Array.isArray(info.parts) && info.parts.length) {
        COMBAT_MELEE_BODY_TIERS = loadCombatMeleeTiersFromSpec() || COMBAT_MELEE_BODY_TIERS;
        module.exports.BODY_TIERS = COMBAT_MELEE_BODY_TIERS.map(function (tier) { return tier.slice(); });
  ...
  var fallback = CoreSpawn.pickLargestAffordable(COMBAT_MELEE_BODY_TIERS, available || 0);
...
module.exports.getSpawnSpec = function (room, ctx) {
  ...
  if (TaskSpawn && typeof TaskSpawn.getBodyFor === 'function') {
    try {
      info = TaskSpawn.getBodyFor('CombatMelee', room, context) || null;
    } catch (err) {
      info = null;
    }
  }
