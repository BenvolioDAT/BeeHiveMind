...
// --- Add these near your other tunables ---
var PICKUP_WARN_PCT   = 0.60;  // requestPickup at 60%
var PICKUP_URGENT_PCT = 0.80;  // reinforce at 80%

var EMERGENCY_EVAC_ENABLED = false; // optional miner haul-home if jammed
...
var TaskLuna = {
  ...
  updateReturnState: function(creep){
    ...
    if (creep.memory._containerMode) {
      creep.memory.returning = false;
      return;
    }
    // Original carry toggles (kept for non-container phases)
    if (!creep.memory.returning && creep.store.getFreeCapacity(RESOURCE_ENERGY)===0) {
      creep.memory.returning=true; debugSay(creep, '⤴️RET');
    }
    if (creep.memory.returning && creep.store.getUsedCapacity(RESOURCE_ENERGY)===0) {
      creep.memory.returning=false; debugSay(creep, '⤵️WK');
    }
  },
  ...
  // Flag + controller visuals you already had
  ...
  // Container discovery around the source
  var seatInfo = _findContainerOrSiteNearSource(creep.room, src);
  var container = seatInfo.container;
  var csite     = seatInfo.csite;

  // Mark container state in memory
  _updateContainerMemory(src, container, getHomeName(creep));

  // Enter "container mode" if container is present or we're sitting the seat to build one
  creep.memory._containerMode = !!(container || csite);

  // If container exists: sit on it, dump & harvest
  if (container) {
    // store container id in memory for Truckers
    srec.container = srec.container || {};
    srec.container.containerId = container.id;
    ...
    // Transfer any carried energy first
  ...
  // No container yet: if there's a container site at seat, build it; otherwise lay one down.
  if (csite && csite.structureType === STRUCTURE_CONTAINER) {
    // If we have energy, build; else harvest to get build fuel
    if (creep.store.getUsedCapacity(RESOURCE_ENERGY) > 0) {
      debugSay(creep, '🔨CON');
  ...
  // No container or site: pick a good seat and place the site
  var seat = _pickSeatPosNearSource(src) || src.pos;
  if (!seat.x) seat = src.pos; // paranoia

  // If we're on the seat tile already, try to create a container site; else move to it.
  ...
  // Try to place container site if site limit allows
  ...
  // While waiting, harvest to pocket (so we can build next tick)
  debugSay(creep, '⛏️SRC');
  debugDraw(creep, src, CFG.DRAW.SRC_COLOR, "SRC");
