...
const TradeEnergy = {
  run(room) {
    ...
    const pick = pickBestEnergyOrder(room);
    if (!pick) return;

    // Shrink to respect the per-deal cap, then trim further to what the
    // terminal can actually send after fees while keeping its reserve.
    const desired = Math.min(pick.amount, CFG.MAX_PER_DEAL);
    const amount = fitAmountToTerminal(room, pick, desired);
    if (amount <= 0) return;
...
function pickBestEnergyOrder(room) {
  ...
  for (let i = 0; i < candidates.length; i++) {
    ...
    const cap = Math.min(order.amount, CFG.MAX_PER_DEAL);
    const test = Math.max(1000, Math.min(cap, 10000));
    const eff = effectiveCreditsPerEnergy(order, room.name, test);
...
function executeDeal(room, order, amount) {
  ...
  const res = Game.market.deal(order.id, amount, room.name);
  if (res === OK) {
    markTraded(room);
    const fee = Game.market.calcTransactionCost(amount, room.name, order.roomName);
    console.log(
