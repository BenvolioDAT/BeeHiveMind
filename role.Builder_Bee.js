...
var roleBuilder_Bee = {
  ...
  run: function (creep) {
    ...
    if (creep.memory.building) {
      ...
      if (targets.length) {
        ...
        targets.sort((a, b) => (roleBuilder_Bee.siteWeights[b.structureType] || 0) - (roleBuilder_Bee.siteWeights[a.structureType] || 0));
        ...
        roleBuilder_Bee.buildPredefinedStructures(creep);
        roleBuilder_Bee.upgradeController(creep);
      }
    }
  ...
  buildPredefinedStructures: function (creep) {
    for (var i = 0; i < roleBuilder_Bee.structurePlacements.length; i++) {
      var placement = roleBuilder_Bee.structurePlacements[i];
      ...
      if (
        ...
        roleBuilder_Bee.buildStructures(creep, targetPosition, placement.type);
      }
    }
  },
  ...
  buildStructures: function (creep, targetPosition, structureType) {
    ...
    if (
      roleBuilder_Bee.structureLimits[structureType] &&
      roleBuilder_Bee.countStructures(creep.room, structureType) >= roleBuilder_Bee.structureLimits[structureType]
    ) {
      return;
    }
