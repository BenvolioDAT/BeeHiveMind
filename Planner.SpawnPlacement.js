...
function isTileAvailable(room, x, y) {
  ...
  // Reject any tile already occupied by permanent structures other than roads/ramparts.
  ...
  // Skip tiles already earmarked for construction to avoid stacking sites.
  var sites = room.lookForAt(LOOK_CONSTRUCTION_SITES, x, y);
  if (sites && sites.length > 0) return false;
  // Do not cover resource nodes or minerals which would block harvesting.
